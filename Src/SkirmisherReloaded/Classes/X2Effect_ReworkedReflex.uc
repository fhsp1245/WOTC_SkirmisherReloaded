...
function ModifyTurnStartActionPoints(XComGameState_Unit UnitState, out array<name> ActionPoints, XComGameState_Effect EffectState)
{
	...
	if (UnitState.GetUnitValue('SkirmisherReflexValue', ReflexValue))
	{
		if (ReflexValue.fValue > 0)
		{
			...
			UnitState.ClearUnitValue('SkirmisherReflexValue');
		}
	}
}
...
static function EventListenerReturn ReworkedReflexListener(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	...
	local UnitValue ReflexValue, DisableReflexValue;
	...
	if (AbilityState.IsAbilityInputTriggered() && AbilityState.GetMyTemplate().Hostility == eHostility_Offensive)
	{
		...
		if (AbilityContext != none && AbilityContext.InterruptionStatus != eInterruptionStatus_Interrupt)
		{
			if (AbilityContext.InputContext.PrimaryTarget.ObjectID == ApplyEffectParameters.TargetStateObjectRef.ObjectID)
			{
				...
				//Disable Reflex while Interrupt or Battlelord has activated.
				if (TargetUnit.GetUnitValue('ReflexDisabled', DisableReflexValue))
					return ELR_NoInterrupt;
				...
				{
					TargetUnit.GetUnitValue('InstantReflexValue', ReflexValue);
					if (TargetUnit.IsAbleToAct() && ReflexValue.fValue == 0)
					{
						...
						TargetUnit.SetUnitFloatValue('InstantReflexValue', 1, eCleanup_BeginTurn);
					}
				}
				...
				{
					TargetUnit.GetUnitValue('SkirmisherReflexValue', ReflexValue);
					if (ReflexValue.fValue == 0)
					{
						...
						TargetUnit = XComGameState_Unit(NewGameState.ModifyStateObject(TargetUnit.Class, TargetUnit.ObjectID));
						TargetUnit.SetUnitFloatValue('SkirmisherReflexValue', 1, eCleanup_BeginTactical);
					}
				}
